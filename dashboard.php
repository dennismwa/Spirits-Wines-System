...
// Get today's stats
$today = date('Y-m-d');
$today_sales = $conn->query("SELECT COUNT(*) as count, COALESCE(SUM(total_amount), 0) as total FROM sales WHERE DATE(sale_date) = '$today'")->fetch_assoc();

// Get this month's stats
$this_month = date('Y-m');
$month_sales = $conn->query("SELECT COUNT(*) as count, COALESCE(SUM(total_amount), 0) as total FROM sales WHERE DATE_FORMAT(sale_date, '%Y-%m') = '$this_month'")->fetch_assoc();

// Get low stock products
$low_stock = $conn->query("SELECT COUNT(*) as count FROM products WHERE stock_quantity <= reorder_level AND status = 'active'")->fetch_assoc();
...
<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <div class="p-6 border-b">
        <div class="flex items-center justify-between">
            ...
            <a href="/sales" class="text-primary hover:underline text-sm font-medium">View All</a>
        </div>
    </div>
